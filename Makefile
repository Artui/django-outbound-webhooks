...
lint:
	uv run ruff check .
	uv run ty check django_outbound_webhooks
...
# Release pipeline. The version lives in django_outbound_webhooks/version.py (pyproject
...
RELEASE_PACKAGE_NAME := django-outbound-webhooks
RELEASE_VERSION_FILES := django_outbound_webhooks/version.py|^__version__[^=]*= *
